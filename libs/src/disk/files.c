    ...
    while (1)
    {
        ...
        for (uint32_t sec = 0; sec < sec_per_clus; sec++)
        {
            ...
            while(1)
            {
                ...
                memcpy(entry, &entries[i++], sizeof(FAT_entry_t));
                ...
                else
                {
                    ...
                    if(strcmp(name, info.name) == 0)
                    {
                        ...
                        if(extension == NULLPTR || (strcmp(extension, info.extension_begin) == 0))
                        {
                            // tty_printf("---FOUND '%s'---\n", name);
                            return FILE_FOUND;
                        }
                ...
                if(recursive)
                {
                    uint32_t ret = fat32_find_file(entry, child_cluster, name, extension, recursive);
                    if(ret == FILE_FOUND)
                    {
                        return FILE_FOUND;
                    }
...
/*
void fat32_load_file(FAT_entry_t* entry, char* buffer, uint32_t max_size)
{
    uint8_t sec_per_clus = fat32_get_sec_per_clus();

    uint32_t first_sector = fat32_first_sector_of_cluster(entry->cl);
    ...
    while (FAT_CLUSTER_IS_VALID(cluster) && remaining > 0)
    {
        ...
        {
            fat32_load_sector(sector_buf, first_sector + s);
        ...
        cluster = fat32_next_cluster(NULL, cluster);
    }

    return file->size - remaining;
}
*/